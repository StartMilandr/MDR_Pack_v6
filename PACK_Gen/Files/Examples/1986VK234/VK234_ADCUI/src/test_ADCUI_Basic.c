...
static void Test_ChangeCh(void);
...
{  
  ...
  MDR_ADCUI->CTRL1 = ( 0
                    ...
                    | MDR_ADCUI_CTRL1_I1EN_Msk
                    | MDR_ADCUI_CTRL1_V1EN_Msk
                    //| MDR_ADCUI_CTRL1_I2EN_Msk
                    //| MDR_ADCUI_CTRL1_V2EN_Msk
...
{  
  ...
  CfgBase.IBoost     = MDR_On;
  CfgBase.updateRMS  = ADCUI_RMS_VcrsZero;
  
  Cfg.cfgBase = &CfgBase;  
  Cfg.cfgNoLoad = NULL;     //  Уровни не учитываемой энергии
  Cfg.cfgV      = NULL;     //  Отслеживание событий в каналах напряжения
  Cfg.cfgCalibr = NULL;     //  Калибровки для каналов  
  Cfg.cfgChop   = NULL;     //  Режим чоппера (только для К234)
...
static void Test_InitByCfgReg(void)
{
  ...
  CfgRegs.F1.CTR_b.I0_SumOff = MDR_On;
  CfgRegs.F2.CTR_b.I0_SumOff = MDR_On;
...
{
  CfgChannel_Inc();
  ADCUI_SwitchSelDAT(Cfg_ActiveCh, Cfg_ActiveDataID);
  Test_Exec();
}
...
{
  uint32_t data = 0;
  data = ADCUI_GetResult(Cfg_ActiveCh, Cfg_ActiveDataID);
  LCD_ShowResult(data, Cfg_ActiveCh, Cfg_ActiveDataID);
}
