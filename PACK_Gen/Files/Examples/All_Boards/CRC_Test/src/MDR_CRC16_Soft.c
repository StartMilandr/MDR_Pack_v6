...
static const uint16_t Modbus_Crc16Table[256] = {
...
uint16_t MDR_CRC16_SoftModbus(uint32_t count, uint8_t *data)
{
    uint16_t crc = 0xFFFF;

    while (count--)
    	crc = (crc >> 8) ^ Modbus_Crc16Table[(crc & 0xFF) ^ *data++];
