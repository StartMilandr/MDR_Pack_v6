...
uint32_t pageData[MDR_EEPROM_PAGE_WORD_COUNT];
void     ReadPageData(MDR_EEPROM_PAGE page, MDR_EEPROM_MEM bank);
void     ReadPages(MDR_EEPROM_PAGE pageEnd, MDR_EEPROM_MEM bank);

uint32_t FindClearData(MDR_EEPROM_PAGE page, MDR_EEPROM_MEM bank);
...
static MDR_EEPROM_MEM   activeBank = EEPROM_IFREN_Main;
  ...
  #define DATA_PAGE_MAIN   EEPROM_Page255 //MDR_EEPROM_LastPage_Main  // Любая, например последняя страница.
  // В Info памяти лежит начальный загрузчик на 341 32-битное слово, чтобы его не затереть необходимо использовать последнюю страницу.
  // Загрузчик прошивается FLM файлом, при записи любой программы. Поэтому затирать его можно.
  #define DATA_PAGE_INFO   EEPROM_Page7 //MDR_EEPROM_LastPage_Info
#endif
 ...
 // MDR_DebugerProtectDelay();
  ...
  while (1)
  {
    ...
    if (MDRB_BntClicked_Right(true))    
    {
      ...
      if (activeBank == EEPROM_IFREN_Main) 
      {
        MDR_RAM_EEPROM_ErasePage(DATA_PAGE_MAIN, MDR_EEPROM_Main);      
        IncRunIndex(RUN_IND_Main);
        result = WritePage(DATA_PAGE_MAIN, MDR_EEPROM_Main, GetRunIndex(RUN_IND_Main));
      }
      else
      {
        ReadPages(EEPROM_Page8, MDR_EEPROM_Info);
        MDR_RAM_EEPROM_ErasePage(DATA_PAGE_INFO, MDR_EEPROM_Info);
        ReadPages(EEPROM_Page8, MDR_EEPROM_Info);
        //FindClearData(DATA_PAGE_INFO, MDR_EEPROM_Info);
...
TestResult WritePage(MDR_EEPROM_PAGE page, MDR_EEPROM_MEM bank, uint32_t runIndex)
{
  ...
  //  runIndex - чтобы данные менялись с каждым запуском
  for (i = 0; i < MDR_EEPROM_PAGE_SIZE; i += 4)
  {
    dataRD = MDR_RAM_EEPROM_Read(addr + i, bank);
    if (dataRD != MDR_EEPROM_CLEAR_VALUE)
    {
//      MDR_BKP_ClockOn();
//      MDR_BKP->REG_0E_b.FPOR = MDR_Off;
//      MDR_Delay(100);
//      MDR_BKP->REG_0E_b.FPOR = MDR_On;
      return tst_Err_Erase;
    }
    
    dataWR = GetTestValue(addr + i + runIndex);
    MDR_RAM_EEPROM_Write(addr + i, dataWR, bank);
  }
...
uint32_t FindClearData(MDR_EEPROM_PAGE page, MDR_EEPROM_MEM bank)
{
  ...
  for (i = 0; i < (MDR_EEPROM_PAGE_SIZE * 16); i += 4)
  {
    dataRD = MDR_RAM_EEPROM_Read(addr + i, bank);
    if (dataRD != MDR_EEPROM_CLEAR_VALUE)
    {
...
void ReadPageData(MDR_EEPROM_PAGE page, MDR_EEPROM_MEM bank)
{
  ...
  for (i = 0; i < MDR_EEPROM_PAGE_WORD_COUNT; ++i)
  {
    pageData[i] = MDR_RAM_EEPROM_Read(addr, bank);
    addr += 4;
  }
}
...
void ReadPages(MDR_EEPROM_PAGE pageEnd, MDR_EEPROM_MEM bank)
{
  ...
  for (page = EEPROM_Page1; page <= pageEnd; ++page)
  {
    ReadPageData(page, bank);
  }
