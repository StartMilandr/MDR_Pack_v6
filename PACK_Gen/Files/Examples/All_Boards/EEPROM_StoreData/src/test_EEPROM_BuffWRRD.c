#include "test_EEPROM_BuffWRRD.h"
  ...
  Вместо этого необходимо подключить модуль test_EEPROM_BuffWRRD_CM0.c!
#endif
...
#define DATA_PAGE_MAIN   EEPROM_Page7  // Любая из страниц кроме начальных, в которых лежит сама эта программа.
#define DATA_PAGE_INFO   EEPROM_Page1  // Info памяти всего одна страница

//  Тестирование записи буфера длиной 1 страницу
#define BUFF_WORD_COUNT  MDR_EEPROM_PAGE_WORD_COUNT
...
static void TestInit(void)
{
  pageAddrMain = MDR_EEPROM_PageToAddr(DATA_PAGE_MAIN, MDR_EEPROM_Main);
  pageAddrInfo = MDR_EEPROM_PageToAddr(DATA_PAGE_INFO, MDR_EEPROM_Info);
}
