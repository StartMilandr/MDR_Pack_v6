...
MDR_HSR_RedBoxCfg redboxCfg = {
  ...
  .forwMode = MDR_HSR_RedBox_HSR_SAN,
	.lanID = MDR_HSRPRP_LanA,
};
...
{   
  ...
  MDR_Eth_Init(MDR_ETH3, &initCfg);
  MDR_Eth_Start(MDR_ETH3);
	MDR_ETH_NVIC_Enable(MDR_ETH3, 0);  
    ...
    if (MDRB_BntClicked_Up(true))
		{
			...
			if (frameItem != NULL)
			{
				...
				//	Заполняем frameTX
				MDR_ETH_Debug_FillTestFrameTX(frameTX, TEST_FRAME_LEN, redboxCfg.nodeCfg.hostMAC, destMAC);			
...
MDR_LOG_Add_def(frameItem->_frmIndex);
    ...
    if (MDRB_BntClicked_Right(true))
    {
			...
			MDR_HSR_SendSuperFrame(&redboxCfg);
    #else
      //MDR_HSR_SendSuperFrame(&cfgNode);
		...
		{
		  MDR_ETH_Debug_SendTestFrameBroadcast(MDR_ETH1, TEST_FRAME_LEN, redboxCfg.nodeCfg.hostMAC);
		}    

#if MDR_HSR_REDBOX_EN		
    //	Process HSR DANH mode    
		MDR_HSR_ProcessRedBox(&redboxCfg, HostProcessReceivedFrame);
		//	Process Timed tasks
		MDR_HSR_ProcessNextTimedTask(&redboxCfg, MDR_HSRPRP_GetHostTime);
#else    
		//	Process HSR DANH mode    
...
void ETH3_IRQHandler(void)
{
  ETHx_HSRPRP_IRQHandler(MDR_ETH3, ETH3_HSRPRP_PORT);
MDR_LOG_Add_def(LOG_PORTB_RX);		
}
