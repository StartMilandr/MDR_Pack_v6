...
BaudRateStatus UART_Init ( MDR_UART_TypeDef* UARTx,
						   UART_InitTypeDef* UART_InitStruct, uint32_t PER_Uart_FreqHz )
{
	...
	cpuclock = PER_Uart_FreqHz;

	tmpreg = MDR_CLOCK->UART_CLOCK;
...
/**
  ...
  * @retval None
  */
void UART_IrDAConfig(MDR_UART_TypeDef* UARTx, uint32_t UART_IrDAMode, uint32_t PER_Uart_FreqHz)
{
  /* Check the parameters */
  assert_param(IS_UART_ALL_PERIPH(UARTx));
  ...
  {
    UARTx->ILPR = PER_Uart_FreqHz / IrLPBaud16;
  }
...
void UART_BRGInit(MDR_UART_TypeDef* UARTx, uint32_t UART_BRG)
{
  ...
  if (UARTx == MDR_UART1)
  {
    tmpreg |= MDR_RST_UART__UART1_CLK_EN_Msk;
    tmpreg &= ~MDR_RST_UART__UART1_BRG_Msk;
    tmpreg |= UART_BRG;
  }
  else if (UARTx == MDR_UART2)
  {
    tmpreg |= MDR_RST_UART__UART2_CLK_EN_Msk;
    tmpreg &= ~MDR_RST_UART__UART2_BRG_Msk;
    tmpreg |= (UART_BRG << 8);
  }
  MDR_CLOCK->UART_CLOCK = tmpreg;
}
