...
{
  regADC &= ~ADC_CLEAR_C2;
  regADC |= VAL2FLD_Pos(selC2, MDR_RST_ADC__ADC_C2_SEL) 
...
static void ADC_Set_C1_loc(MDR_CLK_ADC_C1 selC1, MDR_CLK_DIV_256 divClk)
{
  ...
  regADC |= VAL2FLD_Pos(selC1, MDR_RST_ADC__ADC_C2_SEL);
...
void MDR_ADC_SetClockEx_InputUSB(MDR_CLK_SEL_HSIE2 selInp, MDR_CLK_DIV_256 divClk)
{
  ...
  MDR_CLOCK->USB_CLOCK = regUSB | VAL2FLD_Pos(selInp, MDR_RST_USB__C1_SEL);  
