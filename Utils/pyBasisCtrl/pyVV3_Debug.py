...
class PyWindowDebug_VV3(QtWidgets.QWidget, Ui_Form):
    # Constructor
    def __init__(self, parent=None):
        super(PyWindowDebug_VV3, self).__init__(parent)       
        # Setup UI and Show
        self.setupUi(self)
        #Tables
        ...
        self.btDebug1.setText('Read ALL')
        self.btDebug1.clicked.connect(self.ReadALL)
        ...
        self.comCLI = None
        self.saveFileName = str(Path().absolute()) + '/saves/DebugVV3.ini'
        self.RestoreWidgets()
    ...
    def WriteReg(self):
        ...
        if ind != None:
            ...
            addr = int(VV3_RegsDict[regName])
            try:
                value = int(self.uiEdValue[ind].text(), 16)
                self.comCLI.WriteRegList_VV3([addr], [value])
                self.txtEdit.append('{0:10s}({1:4s})= {2:4s}'.format(regName, hex(addr), hex(value)))
            except Exception:
    ...
    def ReadRegAndShow(self, regName, addr):
        try:
            values = self.comCLI.ReadRegList_VV3([addr])
            if len(values) > 0:
                self.txtEdit.append('{0:10s}({1:4s}): {2:4s}'.format(regName, hex(addr), hex(values[0])))
    ...
    def ReadALL(self):
        self.txtEdit.append('   5600BB3 Registers:')
        for regName, addr in VV3_RegsDict.items():
            self.ReadRegAndShow(regName, addr)
